...
export default function Auditor() {
  ...
  const downloadReport = async () => {
    ...
    // No inline watermark needed - samavedaWatermark PNG is injected by capturePdfFromHtml
    const watermarkSvg = '';
    const pageWatermark = '';
    ...
    // ── COVER PAGE ──
    const coverPage = `<div style="position:relative;min-height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;background:#ffffff;padding:60px 80px;page-break-after:always;">${watermarkSvg}<div style="position:relative;z-index:1;text-align:center;width:100%;"><div style="border:1px solid #e2e8f0;border-radius:30px;display:inline-block;padding:8px 24px;margin-bottom:60px;"><span style="font-size:9pt;letter-spacing:.2em;color:#64748b;font-family:Georgia,serif;">C O N F I D E N T I A L &nbsp;— &nbsp;F O R E N S I C &nbsp;A U D I T &nbsp;R E P O R T</span></div><h1 style="font-size:28pt;font-weight:300;color:#0f172a;font-family:Georgia,serif;margin:0 0 16px;">Forensic Audit Analysis</h1><p style="font-size:14pt;color:#64748b;font-family:Georgia,serif;font-style:italic;margin:0 0 60px;">Independent Due Diligence &amp; Risk Assessment</p><p style="font-size:11pt;color:#94a3b8;letter-spacing:.15em;font-family:Georgia,serif;">SAMAVEDA CAPITAL</p></div></div>`;

    // ── TABLE OF CONTENTS ──
    const tocPage = `<div style="position:relative;min-height:60vh;padding:48px 64px;page-break-after:always;">${pageWatermark}<div style="position:relative;z-index:1;"><h2 style="font-size:18pt;font-weight:700;color:#0f172a;margin:0 0 32px;font-family:Georgia,serif;">📋 Table of Contents</h2><table style="width:100%;border-collapse:collapse;">${[['1.','Executive Summary & Risk Score Breakdown','Section 1'],['2.','Revenue Reconciliation Analysis','Section 2'],['3.','Financial Red Flags — Detailed Findings','Section 3'],['4.','Cash Flow & Fund Siphoning Analysis','Section 4'],['5.','Document Authenticity & Integrity Review','Section 5'],['6.','Temporal & Timeline Inconsistencies','Section 6'],['7.','Critical Documentation Gaps','Section 7'],['8.','MNC Client Verification','Section 8'],['9.','Risk Matrix & Beneish M-Score Indicators','Section 9'],['10.','Recommendations & Final Verdict','Section 10']].map(([n,t,s])=>`<tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:10px 8px;color:#64748b;font-size:10pt;width:32px;">${n}</td><td style="padding:10px 8px;font-size:11pt;color:#0f172a;">${t}</td><td style="padding:10px 8px;font-size:10pt;color:#94a3b8;text-align:right;">${s}</td></tr>`).join('')}</table><p style="margin-top:40px;font-size:9pt;color:#94a3b8;letter-spacing:.1em;">SAMAVEDA CAPITAL</p></div></div>`;
    ...
    const metricCards = [rj?.claimed_revenue, rj?.actual_revenue, rj?.claimed_valuation, allRedFlags.length>0?`${allRedFlags.length} (${critCount} Critical, ${medCount} Medium)`:null].filter(Boolean);
    const metricLabels = ['CLAIMED REVENUE','ACTUAL REVENUE','CLAIMED VALUATION','RED FLAGS IDENTIFIED'];

    // ── SECTION 1 ──
    const section1 = `<div style="position:relative;padding:48px 64px;page-break-after:always;">${pageWatermark}<div style="position:relative;z-index:1;"><h2 style="font-size:9pt;letter-spacing:.15em;color:#94a3b8;font-weight:400;margin:0 0 4px;">1</h2><h2 style="font-size:20pt;font-weight:700;color:#0f172a;margin:0 0 24px;font-family:Georgia,serif;">Executive Summary</h2><div style="border:1.5px solid #e2e8f0;border-radius:8px;padding:20px 24px;margin-bottom:24px;background:#fafafa;"><p style="font-size:11pt;color:#374151;line-height:1.7;margin:0;">${esc(execSummary)}</p></div>${metricCards.length>0?`<div style="display:grid;grid-template-columns:repeat(${Math.min(metricCards.length,4)},1fr);gap:12px;margin-bottom:24px;">${metricCards.map((v,i)=>`<div style="border:1.5px solid #e2e8f0;border-radius:8px;padding:14px 16px;background:#fff;"><div style="font-size:8pt;letter-spacing:.1em;color:#94a3b8;margin-bottom:4px;">${metricLabels[i]}</div><div style="font-size:14pt;font-weight:700;color:#0f172a;">${esc(String(v))}</div></div>`).join('')}</div>`:''} ${riskBreakdown.length>0?`<h3 style="font-size:12pt;font-weight:700;color:#0f172a;margin:20px 0 12px;">Forensic Risk Score Breakdown</h3><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">${riskBreakdown.map(rb=>`<div style="border:1.5px solid #e2e8f0;border-radius:8px;padding:12px 14px;"><div style="font-size:8pt;letter-spacing:.08em;color:#64748b;margin-bottom:4px;text-transform:uppercase;">${esc(rb.category)}</div><div style="font-size:18pt;font-weight:700;color:#0f172a;">${rb.score??'—'}<span style="font-size:10pt;color:#94a3b8;">/10</span></div><div style="font-size:9pt;color:#64748b;margin-top:4px;">${esc(rb.note??'')}</div></div>`).join('')}</div>`:''}<p style="margin-top:40px;font-size:9pt;color:#94a3b8;letter-spacing:.1em;">SAMAVEDA CAPITAL</p></div></div>`;

    const makeSection = (num: number, title: string, body: string, breakAfter = true) =>
      `<div style="position:relative;padding:48px 64px;${breakAfter?'page-break-after:always;':''}">${pageWatermark}<div style="position:relative;z-index:1;"><h2 style="font-size:9pt;letter-spacing:.15em;color:#94a3b8;font-weight:400;margin:0 0 4px;">${num}</h2><h2 style="font-size:20pt;font-weight:700;color:#0f172a;margin:0 0 16px;font-family:Georgia,serif;">${title}</h2>${body}<p style="margin-top:40px;font-size:9pt;color:#94a3b8;letter-spacing:.1em;">SAMAVEDA CAPITAL</p></div></div>`;

    const s2 = rj?.section2_revenue_reconciliation;
    const section2 = s2 ? makeSection(2,'Revenue Reconciliation Analysis',`${s2.intro?`<p style="color:#374151;margin-bottom:16px;">${esc(s2.intro)}</p>`:''} ${Array.isArray(s2.data_table)&&s2.data_table.length?`<h3 style="font-size:11pt;font-weight:600;margin:0 0 8px;">Revenue Data Across Documents</h3>${renderTable(['Source Document','FY23','FY24','FY25','Observations'],s2.data_table.map((r:any)=>[r.source_document??'',r.fy23??'—',r.fy24??'—',r.fy25??'—',r.observations??'']))}`:''} ${(Array.isArray(s2.red_flags)?s2.red_flags:[]).map((rf:any)=>renderRedFlagBox(rf.title??'',rf.severity??'medium',rf.detail??rf.evidence??'')).join('')}`) : '';
    const s3 = rj?.section3_financial_red_flags;
    const section3 = s3 ? makeSection(3,'Financial Red Flags — Detailed Findings',(Array.isArray(s3.subsections)?s3.subsections:[]).map((sub:any)=>{const mt=Array.isArray(sub.metrics_table)?sub.metrics_table:[];const cols=mt.length?Object.keys(mt[0]):[];return `<h3 style="font-size:12pt;font-weight:700;color:#0f172a;margin:20px 0 10px;border-bottom:1px solid #e2e8f0;padding-bottom:6px;">${esc(sub.title??'')}</h3>${mt.length?renderTable(cols.map((c:string)=>c.replace(/_/g,' ').toUpperCase()),mt.map((r:any)=>cols.map((c:string)=>r[c]??'—'))):''} ${(Array.isArray(sub.red_flags)?sub.red_flags:[]).map((rf:any)=>renderRedFlagBox(rf.title??'',rf.severity??'medium',rf.detail??'',(Array.isArray(rf.implications)?rf.implications:[]).length?`<ul style="margin:8px 0 0 16px;">${(rf.implications as string[]).map(i=>`<li style="font-size:10pt;color:#374151;">${esc(i)}</li>`).join('')}</ul>`:'')).join('')}`;}).join('')) : '';
    const s4 = rj?.section4_cash_flow_analysis;
    const section4 = s4 ? makeSection(4,'Cash Flow &amp; Fund Siphoning Analysis',(Array.isArray(s4.subsections)?s4.subsections:[]).map((sub:any)=>{const tt=Array.isArray(sub.transactions_table)?sub.transactions_table:[];const cols=tt.length?Object.keys(tt[0]):[];return `<h3 style="font-size:12pt;font-weight:700;color:#0f172a;margin:20px 0 10px;">${esc(sub.title??'')}</h3>${tt.length?renderTable(cols.map((c:string)=>c.replace(/_/g,' ').toUpperCase()),tt.map((r:any)=>cols.map((c:string)=>r[c]??'—'))):''} ${(Array.isArray(sub.red_flags)?sub.red_flags:[]).map((rf:any)=>renderRedFlagBox(rf.title??'',rf.severity??'medium',rf.detail??'',(Array.isArray(rf.forensic_indicators)?rf.forensic_indicators:[]).length?`<ul style="margin:8px 0 0 16px;">${(rf.forensic_indicators as string[]).map(i=>`<li style="font-size:10pt;color:#374151;">${esc(i)}</li>`).join('')}</ul>`:'')).join('')}`;}).join('')) : '';
    const s5 = rj?.section5_document_authenticity;
    const section5 = s5 ? makeSection(5,'Document Authenticity &amp; Integrity Review',`${Array.isArray(s5.completeness_matrix)&&s5.completeness_matrix.length?renderTable(['Document','Status','Issue','Risk Impact'],s5.completeness_matrix.map((r:any)=>[r.document??'',r.status??'',r.issue??'',r.risk_impact??''])):''} ${(Array.isArray(s5.red_flags)?s5.red_flags:[]).map((rf:any)=>renderRedFlagBox(rf.title??'',rf.severity??'medium',rf.detail??'')).join('')}`) : '';
    const s6 = rj?.section6_temporal_inconsistencies;
    const section6 = s6 ? makeSection(6,'Temporal &amp; Timeline Inconsistencies',`${Array.isArray(s6.timeline_table)&&s6.timeline_table.length?renderTable(['Document','Date Referenced','Issue','Severity'],s6.timeline_table.map((r:any)=>[r.document??'',r.date_referenced??'',r.issue??'',r.severity??''])):''} ${(Array.isArray(s6.red_flags)?s6.red_flags:[]).map((rf:any)=>renderRedFlagBox(rf.title??'',rf.severity??'medium',rf.detail??'')).join('')}`) : '';
    const s7 = rj?.section7_documentation_gaps;
    const section7 = s7 ? makeSection(7,'Critical Documentation Gaps',`<p style="color:#374151;margin-bottom:16px;">The following documents are entirely absent from the dataroom.</p>${Array.isArray(s7.gaps_table)&&s7.gaps_table.length?renderTable(['Missing Document','Criticality','Why It Matters'],s7.gaps_table.map((r:any)=>[r.missing_document??'',r.criticality??'',r.why_it_matters??''])):''}`) : '';
    const s8 = rj?.section8_mnc_client_verification;
    const section8 = s8 ? makeSection(8,'MNC Client Claim Verification',`${Array.isArray(s8.verifiable_receipts_table)&&s8.verifiable_receipts_table.length?`<h3 style="font-size:11pt;font-weight:600;margin:0 0 8px;">Verifiable Client Receipts</h3>${renderTable(['Client','Amount','Date','Matches Teaser?'],s8.verifiable_receipts_table.map((r:any)=>[r.client??'',r.amount??'',r.date??'',r.matches_teaser??'']))}`:''} ${Array.isArray(s8.findings)&&s8.findings.length?`<div style="margin-top:16px;"><h3 style="font-size:11pt;font-weight:600;margin:0 0 8px;">Critical Client Verification Failures</h3><ul style="margin:0 0 0 18px;">${(s8.findings as string[]).map(f=>`<li style="font-size:10.5pt;color:#374151;margin-bottom:6px;">${esc(f)}</li>`).join('')}</ul></div>`:''}`) : '';
    const s9 = rj?.section9_risk_matrix;
    const section9 = s9 ? makeSection(9,'Risk Matrix &amp; Beneish M-Score Indicators',`${Array.isArray(s9.beneish_indicators)&&s9.beneish_indicators.length?`<h3 style="font-size:11pt;font-weight:600;margin:0 0 8px;">Beneish M-Score — Qualitative Forensic Indicators</h3>${renderTable(['Forensic Indicator','Present?','Evidence'],s9.beneish_indicators.map((r:any)=>[r.indicator??'',r.present??'',r.evidence??'']))}`:''} ${s9.assessment_summary?`<div style="border-left:4px solid #dc2626;background:#fef2f2;padding:14px 16px;margin:16px 0;border-radius:0 6px 6px 0;"><strong style="color:#dc2626;">Assessment: ${s9.indicators_present_count??'?'} of ${s9.total_indicators??7} Forensic Indicators Present</strong><p style="margin:6px 0 0;color:#374151;">${esc(s9.assessment_summary)}</p></div>`:''}`) : '';
    ...
    const section10 = (() => {
      ...
      const verdict = s10?.final_verdict??''; const vd = s10?.final_verdict_detail??''; const dn = s10?.deal_structure_notes??'';
      const cn: string[] = Array.isArray(rj?.coverage_notes)?rj.coverage_notes:[];
      const vc = verdict.includes('DO NOT')?'#dc2626':verdict.includes('CAUTION')?'#d97706':'#16a34a';
      return makeSection(10,'Recommendations &amp; Final Verdict',`${imm.length?`<h3 style="font-size:12pt;font-weight:700;margin:0 0 8px;">Immediate Actions Required (Pre-LOI)</h3><div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:20px;">${imm.map((a:string)=>`<div style="border:1px solid #fee2e2;background:#fef2f2;border-radius:6px;padding:10px 14px;"><span style="font-size:8pt;font-weight:700;color:#dc2626;">IMMEDIATE — CRITICAL</span><p style="margin:4px 0 0;font-size:10pt;color:#374151;">${esc(a)}</p></div>`).join('')}</div>`:''} ${st_.length?`<h3 style="font-size:12pt;font-weight:700;margin:0 0 8px;">Short-Term Actions (High Priority)</h3><div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:20px;">${st_.map((a:string)=>`<div style="border:1px solid #fde68a;background:#fffbeb;border-radius:6px;padding:10px 14px;"><span style="font-size:8pt;font-weight:700;color:#d97706;">SHORT-TERM — HIGH</span><p style="margin:4px 0 0;font-size:10pt;color:#374151;">${esc(a)}</p></div>`).join('')}</div>`:''} ${dn?`<div style="border:1.5px solid #e2e8f0;border-radius:8px;padding:16px 20px;margin-bottom:20px;background:#fafafa;"><h3 style="font-size:11pt;font-weight:700;margin:0 0 8px;">💰 Valuation &amp; Deal Structure</h3><p style="font-size:10.5pt;color:#374151;margin:0;">${esc(dn)}</p></div>`:''} ${verdict?`<div style="border:2px solid ${vc};border-radius:8px;padding:20px 24px;text-align:center;margin-top:24px;"><div style="font-size:11pt;font-weight:700;color:${vc};letter-spacing:.1em;margin-bottom:8px;">FINAL RECOMMENDATION</div><div style="font-size:18pt;font-weight:800;color:${vc};margin-bottom:12px;">${esc(verdict)}</div><p style="font-size:10.5pt;color:#374151;margin:0;">${esc(vd)}</p></div>`:''} ${cn.length?`<div style="margin-top:24px;padding:12px 16px;background:#f8fafc;border-radius:6px;border:1px solid #e2e8f0;"><strong style="font-size:10pt;color:#64748b;">Coverage Notes</strong><ul style="margin:6px 0 0 18px;">${cn.map((n:string)=>`<li style="font-size:10pt;color:#64748b;margin-bottom:4px;">${esc(n)}</li>`).join('')}</ul></div>`:''} <div style="margin-top:32px;padding-top:16px;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;"><span style="font-size:9pt;color:#94a3b8;letter-spacing:.1em;">SAMAVEDA CAPITAL</span><span style="font-size:8pt;color:#94a3b8;">Forensic Risk Score: ${riskScore??'—'}/100 · Red Flags: ${allRedFlags.length} · Files: ${auditJob?.processed_files??0}</span><span style="font-size:8pt;color:#94a3b8;">S T R I C T L Y &nbsp; C O N F I D E N T I A L</span></div>`,false);
    })();

    // Fallback: old-format red flags list
    const hasRichSections = !!(s2||s3||s4||s5||s6||s7||s8||s9||s10);
    const oldFormatFallback = (rj&&!hasRichSections&&allRedFlags.length>0)?`<div style="position:relative;padding:48px 64px;">${pageWatermark}<div style="position:relative;z-index:1;"><h2 style="font-size:20pt;font-weight:700;color:#0f172a;margin:0 0 8px;font-family:Georgia,serif;">Detailed Red Flag Analysis</h2><p style="color:#64748b;margin-bottom:24px;">${allRedFlags.length} red flag${allRedFlags.length!==1?'s':''} identified across ${auditJob?.processed_files??0} files</p>${allRedFlags.map((rf:any,idx:number)=>renderRedFlagBox(`${idx+1}. ${esc(rf?.title??'Untitled')}`,rf?.severity??'medium',rf?.what_it_means??rf?.detail??'')).join('')}</div></div>`:'';

    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Forensic Audit Report — ${esc(dataroomName)}</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Helvetica Neue',Arial,sans-serif;font-size:11pt;color:#0f172a;background:#fff;line-height:1.6;}code{background:#f1f5f9;padding:2px 5px;border-radius:4px;font-family:'Courier New',monospace;font-size:9.5pt;}</style></head><body>${coverPage}${tocPage}${section1}${section2}${section3}${section4}${section5}${section6}${section7}${section8}${section9}${section10}${oldFormatFallback}</body></html>`;

    // Use same capturePdfFromHtml as CIM/Teaser — injects samavedaWatermark PNG correctly
    toast({ title: 'Generating PDF...', description: 'This may take a few seconds.' });
    try {
      const safeName = dataroomName.replace(/\s+/g, '_');
      await capturePdfFromHtml(html, samavedaWatermark, `forensic_audit_${safeName}_${new Date().toISOString().slice(0,10)}.pdf`);
  ...
  const previewHtml = useMemo(() => {
    ...
    const sections = [
      ...
      s3?sec(3,'Financial Red Flags — Detailed Findings',(Array.isArray(s3.subsections)?s3.subsections:[]).map((sub:any)=>{const mt=Array.isArray(sub.metrics_table)?sub.metrics_table:[];const cols=mt.length?Object.keys(mt[0]):[];return `<h3 style="font-size:11pt;font-weight:700;color:#0f172a;margin:14px 0 8px;border-bottom:1px solid #e2e8f0;padding-bottom:4px;">${esc(sub.title??'')}</h3>${mt.length?rt(cols.map((c:string)=>c.replace(/_/g,' ').toUpperCase()),mt.map((r:any)=>cols.map((c:string)=>r[c]??'—'))):''} ${(Array.isArray(sub.red_flags)?sub.red_flags:[]).map((rf:any)=>rfb(rf.title??'',rf.severity??'medium',rf.detail??'')).join('')}`;}).join('')):'',
      s4?sec(4,'Cash Flow & Fund Siphoning Analysis',(Array.isArray(s4.subsections)?s4.subsections:[]).map((sub:any)=>{const tt=Array.isArray(sub.transactions_table)?sub.transactions_table:[];const cols=tt.length?Object.keys(tt[0]):[];return `<h3 style="font-size:11pt;font-weight:700;color:#0f172a;margin:14px 0 8px;">${esc(sub.title??'')}</h3>${tt.length?rt(cols.map((c:string)=>c.replace(/_/g,' ').toUpperCase()),tt.map((r:any)=>cols.map((c:string)=>r[c]??'—'))):''} ${(Array.isArray(sub.red_flags)?sub.red_flags:[]).map((rf:any)=>rfb(rf.title??'',rf.severity??'medium',rf.detail??'')).join('')}`;}).join('')):'',
                  ...
                  <div style={{ padding: 20 }}>
                    <Tabs defaultValue="audit" className="space-y-4">
                      ...
                      <TabsContent value="audit" className="space-y-4 mt-0"><div className="space-y-5">
                        ...
                        {auditJob?.report_markdown && <>
                          ...
                          <div ref={reportContentRef} style={{ borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden', height: '50vh' }}>
                            <iframe
                              srcDoc={withWatermark(previewHtml, samavedaWatermark)}
                      ...
                      <TabsContent value="teaser" className="space-y-4 mt-0"><div className="space-y-5">
                        ...
                        {teaserIsRunning && <MiniGamesPanel />}
