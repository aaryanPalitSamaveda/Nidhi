...
function CimGenerationProgress({ isRunning }: { isRunning: boolean }) {
  ...
  useEffect(() => { if (!isRunning) { setProgress(0); return; } setProgress(0); progressRef.current = setInterval(() => { setProgress((p) => { if (p >= 95) return 95; return Math.min(95, p + Math.random() * 3 + 0.5); }); }, 800); return () => { if (progressRef.current) clearInterval(progressRef.current); }; }, [isRunning]);
  useEffect(() => { if (!isRunning) return; const timer = setInterval(() => setTipIndex((i) => (i + 1) % CIM_TIPS.length), 4000); return () => clearInterval(timer); }, [isRunning]);
  if (!isRunning) return null;
  return (
    <div className="sv-card space-y-4" style={{ padding: 20, animation: 'svScaleIn 0.4s ease-out' }}>
      <div className="flex items-center gap-4">
        <div style={{ width: 48, height: 48, borderRadius: 14, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}><FileText style={{ width: 22, height: 22, color: '#1e3a8a', animation: 'svPulse 2s ease-in-out infinite' }} /></div>
        <div style={{ flex: 1 }}><div className="flex items-center justify-between" style={{ marginBottom: 6 }}><span style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>Generating CIM</span><span style={{ fontSize: 11, fontWeight: 600, color: '#1e3a8a' }}>{Math.round(progress)}%</span></div><div className="sv-progress-track"><div className="sv-progress-fill-navy" style={{ width: `${progress}%` }} /></div></div>
      </div>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', borderRadius: 12, background: '#f8fafc', border: '1px solid #e2e8f0' }}><Sparkles style={{ width: 16, height: 16, color: '#1e3a8a', flexShrink: 0, marginTop: 2, animation: 'svPulse 2s ease-in-out infinite' }} /><div><p style={{ fontSize: 10, fontWeight: 600, color: '#1e3a8a', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 2 }}>AI is working</p><p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.5, animation: 'svFadeIn 0.5s ease-out' }} key={tipIndex}>{CIM_TIPS[tipIndex]}</p></div></div>
    </div>
  );
}
...
function TeaserGenerationProgress({ isRunning }: { isRunning: boolean }) {
  ...
  const [progress, setProgress] = useState(0);
  const progressRef = useRef<ReturnType<typeof setInterval> | null>(null);
  useEffect(() => { if (!isRunning) { setProgress(0); return; } setProgress(0); progressRef.current = setInterval(() => { setProgress((p) => { if (p >= 95) return 95; return Math.min(95, p + Math.random() * 4 + 0.8); }); }, 600); return () => { if (progressRef.current) clearInterval(progressRef.current); }; }, [isRunning]);
  useEffect(() => { if (!isRunning) return; const timer = setInterval(() => setTipIndex((i) => (i + 1) % TEASER_TIPS.length), 3500); return () => clearInterval(timer); }, [isRunning]);
  if (!isRunning) return null;
  return (
    <div className="sv-card space-y-4" style={{ padding: 20, animation: 'svScaleIn 0.4s ease-out' }}>
      <div className="flex items-center gap-4">
        <div style={{ width: 48, height: 48, borderRadius: 14, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}><Zap style={{ width: 22, height: 22, color: '#1e3a8a', animation: 'svPulse 2s ease-in-out infinite' }} /></div>
        <div style={{ flex: 1 }}><div className="flex items-center justify-between" style={{ marginBottom: 6 }}><span style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>Generating Teaser</span><span style={{ fontSize: 11, fontWeight: 600, color: '#1e3a8a' }}>{Math.round(progress)}%</span></div><div className="sv-progress-track"><div className="sv-progress-fill-navy" style={{ width: `${progress}%` }} /></div></div>
      </div>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', borderRadius: 12, background: '#f8fafc', border: '1px solid #e2e8f0' }}><Sparkles style={{ width: 16, height: 16, color: '#1e3a8a', flexShrink: 0, marginTop: 2, animation: 'svPulse 2s ease-in-out infinite' }} /><div><p style={{ fontSize: 10, fontWeight: 600, color: '#1e3a8a', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 2 }}>AI is working</p><p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.5, animation: 'svFadeIn 0.5s ease-out' }} key={tipIndex}>{TEASER_TIPS[tipIndex]}</p></div></div>
    </div>
  );
}
...
/* ═══════════════════════════ MAIN COMPONENT ═══════════════════════════ */
/* Auth is handled by the ProtectedRoute wrapper in App.tsx — no internal auth logic needed */
                  ...
                  <div style={{ padding: 20 }}>
                    <Tabs defaultValue="audit" className="space-y-4">
                      ...
                      <TabsContent value="audit" className="space-y-4 mt-0"><div className="space-y-5">
                        ...
                        {auditJob?.report_markdown && <><GamifiedAuditProgress auditJob={auditJob} documents={documents} /><ScrollArea className="h-[50vh]" style={{ borderRadius: 16, border: '1px solid #e2e8f0' }}><div ref={reportContentRef}><div style={{ background: '#fff' }}><div style={{ borderBottom: '1px solid #e2e8f0', background: '#f8fafc', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><div><p style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#94a3b8', fontWeight: 500 }}>Forensic Audit Report</p><p style={{ fontSize: 16, fontWeight: 600, color: '#0f172a', marginTop: 2 }}>{session?.company_name}</p></div><div className="flex gap-2"><span style={{ borderRadius: 999, border: '1px solid #e2e8f0', background: '#fff', padding: '4px 12px', fontSize: 11, color: '#64748b' }}>{auditJob?.processed_files}/{auditJob?.total_files} files</span><span style={{ borderRadius: 999, border: '1px solid #bbf7d0', background: '#f0fdf4', padding: '4px 12px', fontSize: 11, color: '#059669', fontWeight: 500 }}>Completed</span></div></div><div style={{ padding: '20px 24px', position: 'relative' }}><div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: `url(${samavedaWatermark})`, backgroundRepeat: 'repeat', backgroundSize: '350px', opacity: 0.12, zIndex: 10 }} /><div className="relative z-0 prose prose-sm max-w-none break-words prose-h2:text-indigo-700 prose-h3:text-emerald-700 prose-p:text-slate-700 prose-strong:text-slate-900 prose-table:border prose-th:border prose-td:border prose-th:bg-slate-100"><ReactMarkdown remarkPlugins={[remarkGfm]}>{auditJob.report_markdown}</ReactMarkdown></div></div></div></div></ScrollArea></>}
                      </div></TabsContent>
                      <TabsContent value="cim" className="space-y-4 mt-0"><div className="space-y-5">
                        ...
                        {cimIsRunning && <><CimGenerationProgress isRunning={cimIsRunning} /><MiniGamesPanel /></>}
                      ...
                      <TabsContent value="teaser" className="space-y-4 mt-0"><div className="space-y-5">
                        ...
                        {teaserIsRunning && <><TeaserGenerationProgress isRunning={teaserIsRunning} /><MiniGamesPanel /></>}
